  ...
  check (editor) {
    ...
    for (i = 0, len = cursors.length; i < len; i++) {
      ...
      if (matched = text.match(this.regex_block)) {
        ...
        if (matched[3].length) {
          // Get the title value from the regex matched array.
          var title = matched[3];

          // Check for Title Trim option, if true trim the title.
          if (this.settings.title_trim) {
            title = title.trim();
          }

          // Check for Title Uppercase option, if true conver to uppercase.
          if (this.settings.title_uppercase) {
            title = title.toUpperCase();
          }

          // Create title line.
          editor.insertText(';'.repeat(this.settings.body_size) + ' '.repeat(this.settings.header_space) + title);
          editor.insertNewline();

          // Check for Header Posttitle option, if true creat one additional header after the title line.
          if (this.settings.header_posttitle) {
            editor.insertText(';'.repeat(this.settings.header_size));
            editor.insertNewline();
          }
        }
